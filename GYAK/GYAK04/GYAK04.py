...
def get_column(df:pd.DataFrame,szoveg:str)->pd.Series:
    testDf=df.copy()
    return df[szoveg]
...
def population_density(df:pd.DataFrame)->pd.DataFrame:
    testdf=df.copy()
    density=testdf['population']/testdf['area']
    testdf['density']=density
    return testdf
...
def plot_population(df:pd.DataFrame)->plt.Figure:
    ...
    fig, ax = plt.subplots()
    ax.bar(x,y)
    ax.set(ylabel='Population (millions)', title='Population of Countries')
